...
The following query is typed into the BigQuery SQL Editor for each table:
...
BigQuery will try to estimate the amount of data it will read before the query is ran in a text box immediately below the query window:

![external table query](image.png)
...
The External TAble data resides within the external data source, in this case being the GCP Bucket.
...
It is better to use partitioning on a date column and and then cluster by other columns frequently used in query filters/joins.
...
0 Bytes. The `count(*)` value is an aggregation served entirely from the table metadata statistics as opposed to needing to read the table itself.
